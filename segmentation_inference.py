...
params_dict['input_image_names'] = ['ct_soft_tissue.nii.gz']                    #list of input volumes names
params_dict['ground_truth_label_names'] = ['adrenal_binary-label.nii.gz', 'distance_transform_loss_matrix.nii.gz']       #list of ground truth volumes names
...
params_dict['model_outputs_dir'] = params_dict['scratch_dir'] + 'model_outputs_segmentation/'                         #directory where all outputs of the model will be saved (i.e. saved model weights, optimizer, text file, etc.)
...
params_dict['predicted_label_name'] = ['model-label.nii.gz', 'probability-mask.nii.gz']                  #binarized predicted output save name and probability mask save name
...
params_dict['patch_size'] = [224,224,32]            #size of patches to get from input volumes
params_dict['sample_background_prob'] = 0.5         #probability of sampling background class during patching
params_dict['batch_size'] = [2, 2, 8]               #batch size during training, validation, and prediction
params_dict['learning_rate'] = 0.2                  #initial learning rate 
...
params_dict['num_patches_per_patient'] = [6, 10]                        #number of patches to extract from each patient in the training set and validation set
...
params_dict['require_positive_class_per_batch'] = True                  #whether all batches are required to have an example of the positive class present (will help stabilize training especially when training at small batch sizes)
...
params_dict['run_augmentations_every_epoch'] = [False, 5]               #whether to run augmentations every epoch; if False, how many epochs before refreshing data cache; only possible if "cache_volumes" option is true
...
params_dict['downsample_for_augmentation'] = [True, 2]                  #whether to downsample the meshgrid for faster augmentation (helpful if augmentation is bottle-necking training script), and if so, the factor by which to downsample the grid
...
params_dict['augmentation_scheduler'] = [True, 5]                                        #whether to ramp up augmentation probability over many epochs; number of epochs to reach full augmentation probability
...
params_dict['erode_dilate_conn_comp'] = [[False, .15, 3], [False, .15, 3], [False, .05]] #only applicable if passing in binary mask as input; whether to use erosion augmentation, probability of erosion, and max number of iterations; whether to use dilation augmentation, probability of dilation, and max number of iterations; whether to delete connected components at random, and probability of deletion
...
params_dict['use_lr_warmup'] = [True, 3, 1000]                          #whether to use learning rate warmup, how many epochs to warmup for, factor smaller than actual learning rate to start warmup period at
...
params_dict['num_epochs_cycle'] = 80                                   #number of epochs in the first cycle
params_dict['factor_decrease_learning_rate'] = 200                      #amount to decrease learning rate by the end of the cycle
...
params_dict['patch_overlap'] = 0.85                                                     #precentage of overlap with neighboring patches (i.e. .75 means 75% overlap)
...
params_dict['predict_fast_to_find_roi'] = [True, .1, [7,7,2]]                           #whether to first run inference using small patch_overlap and then predict again using the true patch overlap only on the region of interest that has been detected; how many slices above/below ROI to add in case small patch_overlap is incorrect
...
params_dict['use_attentive_normalization'] = [False, 8, 32, [10,10,15,20,20]]           #whether to use attentive normalization, reduction ratio for bottleneck, minimum size of bottleneck, value of K at each layer of the network
params_dict['loss'] = 'joint_dice_boundary_loss'                                        #loss function (from loss_functions.py file)
...
params_dict['pooling'] = 'StridedConvolution'                                           #pooling operation (choose from 'MaxPool', 'AveragePooling', 'StridedConvolution', 'ProjectionShortcut' (ProjectionShortcut only usable with residual blocks and cannot be used with atrous convolutions or selective kernels since tensorflow does not support convolutions that are both strided and dilated))
...
params_dict['dropblock_params'] = None #[[.8,8,[224,224,32],True], [.8,8,[112,112,16],True], [.8,8,[56,56,8],True], [.8,8,[28,28,8],True], [.8,8,[14,14,8],True]]        #probability for dropblock at each level of network; block size at that level; size of feature map at that level; whether to use SpatialDropout instead (will be significantly faster for larger feature maps since this implementation is rather slow)
...
params_dict['factor_reweight_foreground_classes'] = [1., 1.]      #list of size equal to the number of classes (including background class) (i.e. list of size two for binary task, list of size 4 for task with 3 foreground classes and 1 background, etc) with factor to change weighting of each class in cross entropy losses
